...
def Hedgehog(pdb,fix_selection,pull_selection):
    ...
    #translation of the anchor of the cone to the center of mass od the pulled protein
    for i in range(len(hedgehog)):
        hedgehog[i][0] += pull_COM[0]
        hedgehog[i][1] += pull_COM[1]
        hedgehog[i][2] += pull_COM[2]
...
def Gen_input(name,pdb,psf,vectors,template,sel1,sel2,par1,par2=None,coor=None,vel=None,xsc=None):
    ...
    os.system('cp -r '+str(par1)+' '+str(name)+'/')
    if par2: os.system('cp '+str(par2)+' '+str(name)+'/')
    ...
    fix = u.select_atoms(sel1+' and name CA')
    fix.occupancies = [1 for i in range(len(fix.occupancies))]
    pul = u.select_atoms(sel2+' and name CA')
    pul.tempfactors = [1 for i in range(len(pul.tempfactors))]
    ...
    for v,l in zip(vectors[0],vectors[1]):
        ...
        if par2: new_f.write('parameters	../'+str(par2.split('/')[-1])+'\n')

        for line in f.readlines():
            if line[0:7] != 'structu' and line[0:7] != 'coordin' and line[0:7] != 'bincoor' and line[0:7] != 'binvelo' and line[0:7] != 'extend' and line[0:7] != 'paramet':
              line_new = line
              if line[0:10] == 'outputname': line_new = 'outputname      md'+'\n'
              if line[0:8] == 'SMDfile ': line_new = 'SMDfile ../SMD_constraints.pdb'+'\n' #The files common for each SMD_run are stored in the output directory
              if line[0:7] == 'SMDDir ': line_new = 'SMDDir	'+str(v[0])+' '+str(v[1])+' '+str(v[2])+'\n'
              if line[0:8] == 'consref ': line_new = 'consref ../SMD_constraints.pdb'+'\n'
...
def Gen_run(name,label,template):
    for l in label:
        ...
        for line in f.readlines():
            ...
            if len(line.split())>1 and line.split()[1][0:4] == 'INPF': line_new = 'set INPF='+str(name)+'/SMD_theta_'+str(int(l[0]))+'_phi_'+str(int(l[1]))+'/mdrun.inp'+'\n'
            if line[0:4] == 'INPF': line_new = 'INPF='+str(name)+'/SMD_theta_'+str(int(l[0]))+'_phi_'+str(int(l[1]))+'/mdrun.inp'+'\n'
            if line[0:4] == 'OUTF': line_new = 'OUTF='+str(name)+'/SMD_theta_'+str(int(l[0]))+'_phi_'+str(int(l[1]))+'/mdrun.log'+'\n'

            if len(line.split())>1 and line.split()[1][0:4] == 'OUTF': line_new = 'set OUTF='+str(name)+'/SMD_theta_'+str(int(l[0]))+'_phi_'+str(int(l[1]))+'/mdrun.log'+'\n'
            new_f.write(line_new)
        ...
        print('. '+str(name)+'/SMD_theta_'+str(int(l[0]))+'_phi_'+str(int(l[1]))+'/run.bash') # On a single gpu station, it could be better to run one job after one (the line without & at the end so the next job is waiting for the one to finish)
